...
class Application {
	...
	public:
		...
		void run() {
			...
			WorldObject wo0(model, prog_cube, 1, 2);
			wo0.setTransformation(glm::scale(glm::mat4(1.0f), glm::vec3(0.5f)));

			WorldObject wo1(floor, prog_cube, 3, 4);
			
			WorldObject wo2(model, prog_light);
			wo2.setTransformation(glm::scale(glm::mat4(1.0f), glm::vec3(0.05f)));
			wo2.setPosition({ 3.0f, 2.0f, 3.0f });

			WorldObject wo3(model, prog_cube, 1, 2);
			wo3.setPosition({ -1.0f, 0.0f, 3.0f });

			LightEmitter le0(glm::vec3(0.05f), glm::vec3(0.8f), glm::vec3(1.0f), glm::vec3(1.0f, 0.09f, 0.032f));
